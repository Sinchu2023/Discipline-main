...
Execute Phase 1 now and close only after logging the full ${this.app.formatDuration(phase1)}.`;
        ...
        getDailyMissionTasks() {
          ...
          if (!learningSlots && active) {
            ...
            learningSlots = uncompleted.slice(0, 3).map(d => (d.text || "").split("\n")[0].trim());
          ...
          const d1_topic = getTopic(0, "Deep Work Phase 1");
          const d2_topic = getTopic(1, "Deep Work Phase 2");
          const learn_topic = getTopic(2, "Learning Session");

          const makeTask = (labelPrefix, topic, win, priority, dType, target) => {
              ...
              return {
                 type: "custom", topic, label, progress, done, win, priority, discipline_type: dType,
          ...
          return [
              makeTask("DEEP WORK", d1_topic, [5, 6.25], "HIGH", "STRICT", 75),
              makeTask("TRAIN", "TRAIN", [6.25, 7.5], "HIGH", "STRICT", 75),
              makeTask("DEEP WORK", d2_topic, [7.5, 11], "HIGH", "STRICT", 210),
              makeTask("BUILD", "BUILD", [11, 16], "MEDIUM", "FLEXIBLE", 300),
              makeTask("LEARN", learn_topic, [16, 19.5], "MEDIUM", "FLEXIBLE", 210),
              makeTask("ATOMIC HABITS", "ATOMIC HABITS", [19.5, 22.75], "LOW", "FLEXIBLE", 195),
              makeTask("REST", "REST", [22.75, 29], "HIGH", "STRICT", 375),
          ];
        }
        ...
        // ── Phase 0.5: live loop — 30s, never triggers a full re-render ──────
        startMissionLiveLoop() {
          ...
          this._missionLiveLoopId = setInterval(() => {
            this.applyMissionTimeStates(document.querySelector(".shadow-goal-list"));
          }, 30000);
        }
        ...
        syncMissionFromRoadmap() {
          ...
          container.innerHTML = tasks.map((item, idx) => {
            ...
            const timeId = idx < 4 ? ` id="${timeIds[idx]}"` : "";
            ...
            const labelledBy = idx < 4 ? `${taskIds[idx]} ${timeIds[idx]}` : "";
            ...
            return `<div class="${cls}" data-win-start="${state.startH}" data-win-end="${state.endH}" data-check-id="${this.escapeHtml(checkId)}">` +
