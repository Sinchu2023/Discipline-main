class FirebaseCloudManager {
  ...
  async syncByStorageKey(key, value) {
    ...
    // Write accumulated patch to Firestore (was missing — caused shadowAvg + state to never persist)
    if (Object.keys(patch).length > 0) {
      this.writePatch(patch).catch(e => console.warn("syncByStorageKey patch failed", e));
    }
  }
    ...
    this.timerUnsub = window.FirebaseServices.onSnapshot(
      ...
      (snap) => {
        ...
        if (timerState.status === "paused") {
          // Only stop if cloud startTime matches the current running session.
          // Stale "paused" snapshots (from a previous session) must NOT kill a running timer.
          const cloudStart = Number(timerState.startTime || 0);
          const localStart = this.app.stopwatch.startTime;
          const sessionMatches = cloudStart && localStart && cloudStart === localStart;
          if (!this.app.stopwatch.isRunning || sessionMatches) {
