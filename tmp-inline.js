      ...
      class ShadowEngine {
        ...
        render({
          ...
          this.app.elements["shadow-penalty"].textContent =
            `-${this.app.formatDuration(penalty.minutes)} (${penalty.points}pt)`;
          this.app.elements["shadow-penalty-reason"].textContent = penalty
            .reasons.length
          ...
          if (penalty.points > 0) {
            const updateCountdown = () => {
              ...
              const m = String(Math.floor((ms % 3600000) / 60000)).padStart(
                2,
                "0",
              );
              const s = String(Math.floor((ms % 60000) / 1000)).padStart(
                2,
                "0",
              );
            ...
            expiryEl.textContent =
              "No active penalty • carry-over applies only if today ends as a loss";
          }
          ...
          this.app.elements["shadow-percent"].textContent =
          ...
          this.app.elements["shadow-duel"].textContent =
            scoreDiff > 0
              ? `Leader: You (+${scoreDiff})`
              : scoreDiff < 0
                ? `Leader: Shadow (+${Math.abs(scoreDiff)})`
                : "Leader: Even";
...
Execute Phase 1 now and close only after logging the full ${this.app.formatDuration(phase1)}.`;
        ...
        getDailyMissionTasks() {
          ...
          const today = this.app.getDateString(new Date());
          let learningSlots = this.state.roadmapSlotsDate === today ? this.state.roadmapSlots : null;
          ...
          const makeTask = (labelPrefix, topic, win, priority, dType, target, phase, secondary = false) => {
          ...
          return [
            makeTask("IB CORE", "CA + Reasoning + Quant", [5, 6.25], "HIGH", "STRICT", 75, "Morning"),
            makeTask("TRAIN", "Workout / Jog", [6.25, 7.5], "HIGH", "STRICT", 75, "Morning", true),
            makeTask("BATH + BREAKFAST", "Bath + Breakfast", [7.5, 8.25], "LOW", "FLEXIBLE", 45, "Morning", true),
            makeTask("ANALOG SET 1", analog1Topic, [8.25, 10.25], "HIGH", "STRICT", 120, "Core Study"),
            makeTask("BREAK", "Break + Hydration", [10.25, 10.5], "LOW", "FLEXIBLE", 15, "Breaks", true),
            makeTask("ANALOG SET 2", analog2Topic, [10.5, 12.5], "HIGH", "STRICT", 120, "Core Study"),
            makeTask("LUNCH", "Lunch", [12.5, 13.25], "LOW", "FLEXIBLE", 45, "Breaks", true),
            makeTask("BUILD", "Project / Circuits", [13.25, 15.25], "MEDIUM", "FLEXIBLE", 120, "Core Study"),
            makeTask("ANALOG SET 3", analog3Topic, [15.25, 17.25], "HIGH", "STRICT", 120, "Core Study"),
            makeTask("SNACK", "Snack + Break", [17.25, 17.75], "LOW", "FLEXIBLE", 30, "Breaks", true),
            makeTask("IB REVISION", "IB Revision", [17.75, 18.75], "MEDIUM", "FLEXIBLE", 60, "Evening"),
            makeTask("ANALOG REVISION", analogRevisionTopic, [18.75, 19.5], "MEDIUM", "FLEXIBLE", 45, "Evening"),
            makeTask("DINNER", "Dinner", [19.5, 20.25], "LOW", "FLEXIBLE", 45, "Breaks", true),
            makeTask("LIGHT TASKS", "Planning / Light Tasks", [20.25, 21.5], "LOW", "FLEXIBLE", 75, "Evening", true),
            makeTask("SHUTDOWN", "No screens", [21.5, 22], "HIGH", "STRICT", 30, "Evening", true),
            makeTask("REST", "Sleep", [22, 29], "HIGH", "STRICT", 420, "Evening", true),
          ];
        }
        ...
        updateMissionChecklistScore(cache = this._missionStateCache) {
          ...
          const score = Math.round((done / Math.max(1, total)) * 100);
          scoreEl.textContent = `${score}/100`;
        }

        // ── Phase 0.3: targeted single-row DOM update (no innerHTML rebuild) ─
        ...
        syncMissionFromRoadmap({ skipRender = false, rebuild = true } = {}) {
          ...
          const renderTask = (item, idx) => {
            ...
            return `<div class="${cls}" data-win-start="${state.startH}" data-win-end="${state.endH}" data-check-id="${this.escapeHtml(checkId)}"${ariaLabel}>` +
                     `<div class="mission-copy"><span class="mission-title"${labelId}>${idx + 1}. ${this.escapeHtml(item.label || item.topic)}</span>${badge}</div>` +
                     `<input class="mission-check" type="checkbox" data-mission-check-id="${this.escapeHtml(checkId)}" ${state.done ? "checked" : ""} />` +
                   `</div>`;
          };

          const renderPhaseSet = (targetPhases) =>
            phaseOrder
              ...
              .map((phase) => {
                const entries = tasks
                  ...
                  .filter(
                    ({ item }) => (item.phase || "Core Study") === phase,
                  );
                if (!entries.length) return "";
                const primary = entries.filter(({ item }) => !item.secondary);
                const secondary = entries.filter(({ item }) => !!item.secondary);
                return `<section class="mission-phase">` +
                         `<div class="mission-phase-header"><div class="mission-phase-title">${this.escapeHtml(phase)}</div><div class="mission-phase-count">${entries.length} block${entries.length === 1 ? "" : "s"}</div></div>` +
                         (primary.length ? `<div class="mission-phase-items">${primary.map(({ item, idx }) => renderTask(item, idx)).join("")}</div>` : "") +
                         (secondary.length ? `<div class="mission-phase-items secondary">${secondary.map(({ item, idx }) => renderTask(item, idx)).join("")}</div>` : "") +
                       `</section>`;
              })
              .join("");

          // Phase 0.3: structural innerHTML render — ONLY called here, never on interaction
          primaryContainer.innerHTML = renderPhaseSet(["Morning", "Core Study"]);
          if (secondaryContainer) {
            secondaryContainer.innerHTML = renderPhaseSet([
              "Breaks",
              "Evening",
            ]);
          }
        ...
        renderGithubHeatmap() {
          ...
          weeks.forEach(week => {
            ...
            week.forEach(dateStr => {
              ...
              cell.title = `${dateStr}: ${this.app.formatDuration(minutes)}`;
